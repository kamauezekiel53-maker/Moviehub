/* Movie Hub — app.js (complete upgrades)
   - All 10 upgrades implemented
   - Uses corsproxy.io proxy wrapper for TMDb calls
*/

/* =======================
   CONFIG & DOM SELECTORS
   ======================= */
...
function showSkeletons(count = 8) {
  clearGrid(); moviesGrid.classList.add('skeleton-grid');
...
function endpointForSection(section,page=1){
  ...
  if(section==='anime') return `${API_BASE}/discover/movie?with_genres=16&page=${page}`;
...
function renderMovies(list, mediaType='movie'){
  ...
  list.forEach(m=>{
    ...
    const title = escapeHtml(m.title || m.name || m.original_title || '');
...
async function loadSection(section=state.section,page=state.page){
  try{
    ...
    clearSkeletons();
    if(section==='tv_popular'){ renderMovies(data.results, 'tv'); }
    else { renderMovies(data.results, 'movie'); }
...
async function openModal(movieId, mediaType='movie'){
  try{
    ...
    const path = mediaType==='tv' ? `${API_BASE}/tv/${movieId}` : `${API_BASE}/movie/${movieId}`;
    ...
    openTmdb.href = `https://www.themoviedb.org/${mediaType}/${movieId}`;

    // cast with actor click
    modalCast.innerHTML = (data.credits?.cast?.slice(0,8).map(c=>`<div class="cast-item"><img src="${c.profile_path?IMG_BASE+c.profile_path:''}" alt="${escapeHtml(c.name)}"><button class="actor-link" data-id="${c.id}">${escapeHtml(c.name)}</button></div>`).join('')) || '<p class="muted">No cast available.</p>';
    modalCast.querySelectorAll('.actor-link').forEach(b=>b.addEventListener('click',()=>openActorModal(b.dataset.id)));

    // trailers (grid)
    const vidsAll = data.videos?.results || [];
    const yt = vidsAll.filter(v=>v.site==='YouTube');
    modalVideos.innerHTML = yt.length ? yt.map(v=>`<iframe src="https://www.youtube.com/embed/${v.key}" loading="lazy" allowfullscreen></iframe>`).join('') : '<p class="muted">No trailers available.</p>';

    // watchlist button
    const isFav = isInWatchlist(movieId);
    favBtn.textContent = isFav ? '★ In Watchlist' : '☆ Add to Watchlist';
    favBtn.onclick = ()=> { toggleWatchlist({ id: movieId, title, poster: data.poster_path, mediaType }); showToast(isFav ? 'Removed from Watchlist' : 'Added to Watchlist'); };
...
async function openActorModal(personId){
  try{
    ...
    actorBody.innerHTML = `<div style="display:flex;gap:12px"><img src="${data.profile_path?IMG_BASE+data.profile_path:''}" style="width:140px;border-radius:8px"><div><h2>${escapeHtml(data.name)}</h2><p class="muted">${escapeHtml(data.place_of_birth||'')} • Born: ${data.birthday||''}</p><p>${escapeHtml(data.biography||'No biography available.')}</p></div></div><h3>Known For</h3><div class="grid">${(data.movie_credits?.cast||[]).slice(0,8).map(m=>`<div class="card small" data-id="${m.id}"><div class="poster">${m.poster_path?`<img src="${IMG_BASE+m.poster_path}">`:""}</div><div class="card-body"><h4>${escapeHtml(m.title)}</h4></div></div>`).join('')}</div>`;
    actorBody.querySelectorAll('.card.small').forEach(c=>c.addEventListener('click',()=>{ openModal(c.dataset.id,'movie'); closeActorModal(); }));
  }catch(e){ actorBody.innerHTML = `<p class="muted">Failed to load actor.</p>`; console.error(e); }
}
...
/* ===== download links — robust + fallback ===== */
async function loadDownloadLinks(title){
  try{
    ...
    // exact-ish match
    let matches = list.filter(s => s.title && s.title.toLowerCase().includes(title.toLowerCase()));

    // fuzzy fallback
    if(matches.length===0){
      const clean = title.replace(/[^a-z0-9]/gi,'').toLowerCase();
      matches = list.filter(s => s.title && s.title.replace(/[^a-z0-9]/gi,'').toLowerCase().includes(clean));
    }

    // final fallback: show top 6 sources (not automatically embedded)
    if(matches.length===0){
      modalDownload.innerHTML = `<p class="muted">No exact match — showing available sources.</p>` + list.slice(0,6).map(s=>`<div style="margin:10px 0"><strong style="color:var(--accent)">${escapeHtml(s.title||s.quality||'Link')}</strong><br><a class="btn" href="${s.url}" target="_blank" rel="noopener">Open source</a></div>`).join('');
      return;
    }

    modalDownload.innerHTML = matches.map(s=>`<div style="margin:8px 0"><strong style="color:var(--accent)">${escapeHtml(s.quality||s.title||'Link')}</strong><br><a class="btn" href="${s.url}" target="_blank" rel="noopener">Open source (${escapeHtml(s.size||'Unknown')})</a></div>`).join('');
  }catch(e){
    console.error(e);
...
function makeDraggable(el){
  ...
  el.addEventListener('mousemove', e => { if(!isDown) return; e.preventDefault(); const x=e.pageX - el.offsetLeft; const walk=(x-startX)*1.6; el.scrollLeft = scrollLeft - walk; });
}
...
/* ===== search suggestions + search handling ===== */
function debounce(fn, wait=300){ let t; return (...args)=>{ clearTimeout(t); t=setTimeout(()=>fn(...args), wait); }; }
const fetchSuggestions = debounce(async q => {
  if(!q){ suggestionsBox.classList.add('hidden'); return; }
  try{
    const data = await qs(`${API_BASE}/search/multi?query=${encodeURIComponent(q)}&page=1`);
    const list = (data.results || []).filter(r => r.media_type!=='person').slice(0,8);
    if(!list.length){ suggestionsBox.classList.add('hidden'); return; }
    suggestionsBox.innerHTML = list.map(m=>{
      const title = escapeHtml(m.title || m.name || '');
      const sub = m.release_date || m.first_air_date || '';
      return `<div class="suggest-item" data-id="${m.id}" data-type="${m.media_type}">${title} <small class="muted">• ${sub?sub.slice(0,4):''}</small></div>`;
    }).join('');
    ...
    suggestionsBox.querySelectorAll('.suggest-item').forEach(it => it.addEventListener('click', ()=>{
      ...
      suggestionsBox.classList.add('hidden'); searchInput.value = it.textContent.trim();
      openModal(id, type);
    }));
  }catch(e){ suggestionsBox.classList.add('hidden'); console.error(e); }
}, 300);
...
async function doSearch(query, page=1){
  try{
    ...
    const url = `${API_BASE}/search/multi?query=${encodeURIComponent(query)}&page=${page}`;
    const data = await qs(url);
    clearSkeletons();
    // normalize mixed results into movie-like objects
    const normalized = (data.results || []).map(r => ({
      id: r.id,
      title: r.title || r.name,
      name: r.name,
      poster_path: r.poster_path || r.backdrop_path,
      vote_average: r.vote_average || r.vote_average,
...
sectionButtons.forEach(btn=>{
  btn.addEventListener('click', ()=>{
    try {
      ...
      if(sec === 'favorites'){ // open watchlist
        renderWatchlistPanel(); watchlistPanel.classList.remove('hidden'); watchlistPanel.setAttribute('aria-hidden','false');
        return;
      }
      state.section = sec; state.page = 1; state.query = ''; searchInput.value = ''; suggestionsBox.classList.add('hidden');
      loadSection(sec, 1);
...
function renderWatchlistPanel(){
  ...
  watchlistItems.querySelectorAll('.watch-item .remove').forEach(btn => btn.addEventListener('click', ()=>{
    const id = btn.closest('.watch-item').dataset.id;
...
/* ===== player modal (for trailers) ===== */
function openPlayerWithYouTube(key){
  playerContainer.innerHTML = `<iframe width="100%" height="480" src="https://www.youtube.com/embed/${encodeURIComponent(key)}?autoplay=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
  playerModal.classList.remove('hidden'); playerModal.setAttribute('aria-hidden','false'); document.body.style.overflow='hidden';
}
playerModal.querySelector('.player-close')?.addEventListener('click', ()=>{ playerModal.classList.add('hidden'); playerModal.setAttribute('aria-hidden','true'); playerContainer.innerHTML=''; document.body.style.overflow=''; });
...
/* ===== drag-to-scroll support for touch (mobile) ===== */
